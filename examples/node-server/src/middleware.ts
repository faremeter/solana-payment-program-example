...
import { createSettleTransaction, extractTransferData, isValidTransferTransaction, processTransaction, settleTransaction } from './solana';
import fs from "fs"

export const paymentMiddleware = (paymentRequirements: PaymentRequirements) => {
    const connection = new Connection(clusterApiUrl("devnet"), "confirmed");
    const adminKeypair = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(fs.readFileSync("../../keypairs/admin.json", "utf-8"))));
