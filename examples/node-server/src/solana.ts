import { clusterApiUrl, Keypair, MessageV0, PublicKey, SystemProgram, TransactionInstruction, TransactionMessage, TransactionSignature, VersionedMessage, VersionedTransaction } from "@solana/web3.js";
import { Connection } from "@solana/web3.js";
import { CreatePaymentArgs, PaymentRequirements, TransactionVerificationResult, Uint8Array32 } from "./types";
...
export const isValidTransferTransaction = async (
    ...
    const transferIndex = transaction.transaction.message.compiledInstructions.findIndex(
        instruction => {
...
export const extractTransferData = async (
    ...
    const payer = transaction.transaction.message.staticAccountKeys[
        transaction.transaction.message.compiledInstructions[transferIndex].accountKeyIndexes[0]
    ];

    const transferData = transaction.transaction.message.compiledInstructions[transferIndex].data;
...
export const createPaymentTransaction = async (connection: Connection, paymentRequirements: PaymentRequirements, payer: Keypair): Promise<VersionedTransaction> => {
...
export const createSettleTransaction = async (connection: Connection, settleAuthority: Keypair, payer: PublicKey, paymentNonce: number[]): Promise<VersionedTransaction | null> => {
...
export const settleTransaction = async (conneciton: Connection, tx: VersionedTransaction): Promise<{
    success: true
} | {
    success: false,
    err: string
